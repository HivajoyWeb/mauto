...
const { bot2, Saavan, channelId } = require('./telegram');
...
async function updateProgressMessage(chatId, messageId, progressData) {
    ...
    try {
        await bot2.editMessageText(text, {
            chat_id: chatId,
...
async function downloadAndSendSong(songId, progressCallback = null) {
    ...
    try {
        ...
        const sent = await bot2.sendAudio(channelId, filepath, {
            caption,
            title,
            performer: artists,
            duration
...
async function downloadWithProgress(chatId, type, id) {
    ...
    try {
        ...
        if (songs.length === 0) {
            await bot2.sendMessage(chatId, `❌ No songs found for this ${type}!`);
        ...
        const progressMsg = await bot2.sendMessage(chatId, initialText, { parse_mode: 'MarkdownV2' });
        ...
        await bot2.editMessageText(finalText, {
            chat_id: chatId,
            message_id: messageId,
            parse_mode: 'MarkdownV2'
        });
        ...
        await bot2.sendMessage(chatId, `❌ Error: ${err.message}`);
    } finally {
        activeDownloads.delete(downloadKey);
    }
}
...
function setupBotCommands() {
    // /artist command
    bot2.onText(/\/artist\s+(\S+)/, async (msg, match) => {
        const chatId = msg.chat.id;
        const artistId = match[1];

        console.log(`\n📥 Bot command: /artist ${artistId} from ${msg.from.username || msg.from.id}`);

        await bot2.sendMessage(chatId, `🔍 Fetching artist info for ID: \`${artistId}\`...`, { parse_mode: 'Markdown' });
    ...
    // /playlist command
    bot2.onText(/\/playlist\s+(\S+)/, async (msg, match) => {
        const chatId = msg.chat.id;
        const playlistId = match[1];

        console.log(`\n📥 Bot command: /playlist ${playlistId} from ${msg.from.username || msg.from.id}`);

        await bot2.sendMessage(chatId, `🔍 Fetching playlist info for ID: \`${playlistId}\`...`, { parse_mode: 'Markdown' });
    ...
    // /status command
    bot2.onText(/\/status/, async (msg) => {
        const chatId = msg.chat.id;

        if (activeDownloads.size === 0) {
            await bot2.sendMessage(chatId, '✅ No active downloads');
        } else {
            const active = Array.from(activeDownloads.keys()).join('\n• ');
            await bot2.sendMessage(chatId, `📥 *Active Downloads:*\n• ${active}`, { parse_mode: 'Markdown' });
        }
    });

    // /help command
    bot2.onText(/\/help|\/start/, async (msg) => {
        const chatId = msg.chat.id;
        const helpText = `
...
Songs will be sent to the configured channel and saved to database\\.
        `;
        await bot2.sendMessage(chatId, helpText, { parse_mode: 'MarkdownV2' });
    });
